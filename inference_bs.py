...
parser.add_argument("-t", "--testOnly", help = "Only test set items", default="False")
...
if testOnly:
    k = -1 
out_folder = f'predicted_{dataset_name}'

for model_file in tqdm(os.listdir(f'saved_{dataset_name}')):
    os.makedirs(out_folder, exist_ok=True)
    model_name = model_file.split("-")[0]
    if model_name in [x.split('_')[-1].split('.')[0] for x in os.listdir(f'predicted_{dataset_name}')] + ['ERROR_FILE.txt']:
        continue
